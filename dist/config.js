...
// Helpers
const splitList = (s, def = '') => (s || def)
    .split(',')
    .map(x => x.trim())
    .filter(Boolean);
// Payment Methods
export const METHODS_ENABLED_DEFAULT = splitList(process.env.METHODS_ENABLED_DEFAULT || '', 'VENMO,ZELLE,PAYPAL,CASHAPP,APPLE PAY,CRYPTO,CARD').map(s => s.toUpperCase());
export const METHODS_CIRCLE = splitList(process.env.METHODS_CIRCLE || '', 'VENMO,ZELLE,CASHAPP').map(s => s.toUpperCase());
export const METHODS_EXTERNAL_LINK = splitList(process.env.METHODS_EXTERNAL_LINK || '', 'PAYPAL,APPLE PAY,CRYPTO,CARD').map(s => s.toUpperCase());
export const CONFIG = {
    MIN_BUY_IN: 20,
    MAX_BUY_IN: 2000,
