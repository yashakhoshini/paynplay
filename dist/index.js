...
bot.callbackQuery(/^MARKPAID:(.+?):(\d+)$/, async (ctx) => {
    try {
...
async function startWithdrawFlow(ctx) {
    try {
        ...
        if (ownerMethods.includes('VENMO'))
            availableMethods.push({ text: "Venmo", callback: "WITHDRAW_METHOD_VENMO" });
        if (ownerMethods.includes('ZELLE'))
            availableMethods.push({ text: "Zelle", callback: "WITHDRAW_METHOD_ZELLE" });
        if (ownerMethods.includes('CASHAPP'))
            availableMethods.push({ text: "Cash App", callback: "WITHDRAW_METHOD_CASHAPP" });
        if (ownerMethods.includes('PAYPAL'))
            availableMethods.push({ text: "PayPal", callback: "WITHDRAW_METHOD_PAYPAL" });
...
async function handleWithdrawConfirm(ctx) {
    try {
        ...
        try {
            ...
            // Confirm withdrawal
            await confirmWithdrawal(reqId, Number(userId), username, Number(amountUSD), method, tag, requestTimestampISO, fromId);
            // Invalidate cache since we updated the sheet
            invalidateCache();
