export const MSG = {
    welcome: (club) => `Welcome to ${club}! Choose an option:`,
    ...
    playerMatchedPay: (amt, cur, method, recv) => `Matched to an existing cash-out.\n` +
    ...
    playerOwnerPay: (amt, cur, method, handle, extra) => `Pay **${amt} ${cur}** via ${method} to **${handle}**.\n\n` +
        `${extra ?? ""}Then *post your payment screenshot in the group chat*. A loader/owner will confirm.\n`,
    groupCard: (player, amt, cur, method, recv) => `🧾 *Transaction*\n` +
        `Player: ${player}\n` +
        `Amount: *${amt} ${cur}*\n` +
        `Method: ${method}\n` +
        `Pay to: *${recv}*`,
    notAuthorized: "You're not authorized to confirm payments.",
    paidConfirmed: (verifier, iso) => `✅ Paid confirmed by ${verifier} at ${iso}`,
    // Legacy messages for backward compatibility
    ownerPay: (amt, cur, method, handle, extra) => `Pay **${amt} ${cur}** via ${method} to **${handle}**.\n\n${extra ?? ''}`,
    matchedPay: (amt, cur, method, recv) => `Matched to an existing cash-out.\nPay **${amt} ${cur}** via ${method} to **${recv ?? '<ask recipient>'}**.\n\nAfter sending, tap 'Mark Paid'.`,
    markPaid: '✅ Mark Paid',
    buyIn: '💸 Buy-In',
    custom: 'Custom',
    pong: 'pong ✅',
    ...
    groupWelcome: (botUsername) => `🎰 **Pay-n-Play Bot Setup**\n\n` +
    ...
    reminderFirstTime: (botUsername) => `Hi 👋 — you can DM me (@${botUsername}) or type /start here to buy in.`,
    transactionCard: (playerName, amount, currency, method, payeeHandle) => `💸 **Buy-In Request**\n\n` +
        `**Player:** ${playerName}\n` +
    ...
    withdrawSummary: (method, amount, tag) => `Review your withdrawal:\n` +
        `• Method: ${method}\n` +
        `• Amount: $${amount.toFixed(2)}\n` +
        `• Tag: ${tag}\n\n` +
        `Tap "Submit Withdrawal" to send this to loaders.`,
    withdrawSubmitted: "Submitted to loaders. You'll be queued after approval.",
    withdrawCard: (requestId, username, userId, method, amount, tag, timestamp) => `🧾 *Withdrawal Request*\n` +
        `ID: ${requestId}\n` +
    ...
    withdrawConfirmed: (requestId, clickerId) => `✅ Withdrawal ${requestId} confirmed by ${clickerId}. Queued by wait time.`,
