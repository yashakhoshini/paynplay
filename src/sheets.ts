...
export async function appendWithdrawalRow(row: {
  ...
  status: 'QUEUED'|'STALE'|'PAID'|'CANCELLED'|'OWNER_PAID';
  payout_type: 'CIRCLE'|'OWNER';
  notes?: string;
}): Promise<void> {
  try {
    const svc = await client();
...
export async function markOwnerPayoutPaid(payoutId: string, approvedBy: number, notes?: string): Promise<void> {
  try {
    const svc = await client();
...
export async function appendExternalDeposit(row: {
  ...
  username: string;
  amount_usd: number;
  method: string;     // CARD | CASHAPP | APPLEPAY | STRIPE
  reference?: string; // payment_intent id or free text
  created_at_iso: string;
  recorded_by_user_id: string|number;
}): Promise<void> {
  try {
    const svc = await client();
...
export async function upsertLedger(user_id: number, username: string, deltaCents: number, note: string): Promise<number> {
  try {
    const svc = await client();
...
export async function getLedgerBalance(user_id: number): Promise<number> {
  try {
    const svc = await client();
...
export async function markStaleCashAppCircleWithdrawals(staleHours: number): Promise<number> {
  try {
    const svc = await client();
